...
The unsigned `plank-client` DEB was assembled on the dedicated Ubuntu
...
It installs Moonlight under `/usr/libexec/plank/`, the launcher and
...
`PLANK_AV_SYNC_TELEMETRY=1` is set.
...
`/usr/libexec/plank/plank-client`; process mappings resolve all four
bundled FFmpeg 9 libraries from `/usr/libexec/plank/lib/`. The first
...
unit and `/usr/bin/plank-client` launcher.
