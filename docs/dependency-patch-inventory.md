...
`client/moonlight-qt-fork/app/deploy/linux/ffmpeg-patches/0001-hevc-enable-hwaccel-for-identity-gbr.patch`
...
- `scripts/build-client-ffmpeg.sh` checksum-verifies and applies it. A patch
  that is neither applicable nor already applied is fatal.
- `scripts/build-client-package-binaries.sh` checksum-verifies it again and
...
`host/sunshine-fork/third-party/build-deps/patches/` contains the patches
...
fatal. `scripts/build-host-package-binaries.sh` independently requires this
fail-closed contract before configuring a package build.
`scripts/verify-host-dependency-patches.sh` also proves that the generated
...
- `client/moonlight-qt-fork/app/deploy/linux/appimage/` contains an AppImage
  libplacebo patch. PLANK does not build or publish AppImages; the Ubuntu DEB
  uses its qualified system libplacebo.
- `host/sunshine-fork/packaging/linux/patches/{aarch64,x86_64}/` contains CUDA
