...
impl Datagrams<'_> {
    ...
    pub fn send(&mut self, data: Bytes, drop: bool) -> Result<(), SendDatagramError> {
        ...
        let datagram_memory = data.len().saturating_add(size_of::<Datagram>());
        if data.len() > max || datagram_memory > send_buffer_size {
        ...
        if drop {
            self.conn
                .datagrams
                .outgoing
                .make_space_for(datagram_memory, send_buffer_size);
        } else if self
            .conn
            .datagrams
            .outgoing
            .memory_used()
            .saturating_add(datagram_memory)
            > send_buffer_size
        {
...
impl DatagramBuffer {
    fn make_space_for(&mut self, additional_memory: usize, capacity: usize) {
        while self.memory_used().saturating_add(additional_memory) > capacity {
...
mod stationconnect_tests {
    ...
    fn make_space_accounts_for_new_datagram_and_updates_payload_once() {
        ...
        let datagram_memory = 4 + size_of::<Datagram>();
        let capacity = buffer.memory_used() + datagram_memory - 1;

        buffer.make_space_for(datagram_memory, capacity);
