...
#if defined(__GNUC__)
#define PLANK_NOINLINE __attribute__((noinline))
#else
#define PLANK_NOINLINE
#endif

PLANK_NOINLINE void copy_frame(const std::uint8_t *source,
...
PLANK_NOINLINE void unpack_identity_single(
...
PLANK_NOINLINE void unpack_identity_parallel(
    const std::uint8_t *source, int width, int height, int row_pitch,
    std::uint16_t *green, std::uint16_t *blue, std::uint16_t *red,
