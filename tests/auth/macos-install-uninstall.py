...
def main():
    ...
    config_path = state / "host.plist"
    public = plistlib.loads((config_path if config_path.exists() else state / "SignIn/host.plist").read_bytes())
    ...
    files = [directory / name for directory in identities for name in ("key.pem", "key.der", "cert.pem", "cert.der")]
    ...
    def preserved():
        ...
        assert plistlib.loads(config_path.read_bytes()) == public, "Public configuration changed"
    ...
    def ready():
        ...
        while not reachable(public["Port"]):
    ...
    assert not reachable(public["Port"]), "Host listener survived uninstall"
    preserved()
