...
int main(void) {
    ...
    static const uint8_t expected[] = {
        0x53, 0x43, 0x45, 0x31, 0x00, 0x04, 0x00, 0x06,
    ...
    CHECK(sc_datasmash_event_encode(
        SC_DATASMASH_EVENT_CURSOR_POSITION, payload, sizeof(payload),
        encoded, sizeof(encoded), &encoded_size) == 0);
    CHECK(encoded_size == sizeof(expected));
    CHECK(memcmp(encoded, expected, sizeof(expected)) == 0);
    CHECK(sc_datasmash_event_decode(encoded, encoded_size, &decoded) == 0);
    CHECK(decoded.type == SC_DATASMASH_EVENT_CURSOR_POSITION);
    CHECK(decoded.payload_size == sizeof(payload));
    CHECK(memcmp(decoded.payload, payload, sizeof(payload)) == 0);
    CHECK(sc_datasmash_event_decode(encoded, encoded_size - 1, &decoded) == -1);
    return 0;
}
