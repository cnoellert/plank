...
static void check_absolute_mouse(void) {
    ...
    CHECK(plank_transport_input_read_u16(payload) == 1024);
    CHECK(plank_transport_input_read_u16(payload + 2) == 512);
    CHECK(plank_transport_input_read_u16(payload + 4) == 4095);
    CHECK(plank_transport_input_read_u16(payload + 6) == 2159);
}
...
static void check_pen(void) {
    ...
    CHECK(memcmp(payload, expected, sizeof(expected)) == 0);
    CHECK(fabsf(plank_transport_input_read_float(payload + 8) - 0.5f) < 0.000001f);
    CHECK(fabsf(plank_transport_input_read_float(payload + 16) - 0.75f) < 0.000001f);
    CHECK(plank_transport_input_read_u16(payload + 4) == 270);
}
