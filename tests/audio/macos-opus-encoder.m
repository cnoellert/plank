    ...
    @autoreleasepool {
        ...
        for (unsigned mode = 0; mode < 3; ++mode) {
            ...
            PLANKMacOpusEncoder *encoder = [[PLANKMacOpusEncoder alloc] initWithOutput:^BOOL(NSData *packet, CMTime pts) {
                ...
                CHECK(CMTimeCompare(pts, CMTimeMake(480000 - 312 + packets * 240, 48000)) == 0);
            ...
            while (supplied < 96000) {
                ...
                CMTime pts = CMTimeMake(480000 + supplied, 48000);
                if (mode == 2) {
                    ...
                    // accept the full-sample gap checked below.
        ...
        // Every failure is latched; no later valid sample revives the encoder.
        for (unsigned failure = 0; failure < 6; ++failure) {
            __block unsigned packets = 0;
            PLANKMacOpusEncoder *encoder = [[PLANKMacOpusEncoder alloc] initWithOutput:^BOOL(NSData *packet, CMTime pts) {
                (void)packet; (void)pts; ++packets; return failure != 5;
            }];
            ...
            CMTime time = CMTimeMake(480017 + (failure == 0 ? 1 : 0), 48000);
            if (failure == 1) time = CMTimeMake(10, 1); // overlaps previous PCM
        ...
        CHECK(!close(fd));
        printf("macos_opus_encoder=pass checks=%u packets=400 planar_interleaved_uneven_identical=1 gap_stop_latched=1 no_eof_flush=1 priming_frames=312\n", checks);
    }
}
