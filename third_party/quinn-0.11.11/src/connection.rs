...
impl Connection {
    ...
    /// been accepted when a later packet returns an error. Wake the driver
    /// even on that partial-success path. Packet preparation is caller-owned
    ...
    pub fn send_datagram_batch(&self, datagrams: &[Bytes]) -> Result<(), SendDatagramError> {
        for batch in datagrams.chunks(16) {
            ...
            let result = batch.iter().try_for_each(|data| {
                conn.inner.datagrams().send(data.clone(), true).map_err(|e| match e {
                    Blocked(..) => unreachable!(),
