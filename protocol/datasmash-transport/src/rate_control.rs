...
//! StationConnect's rate policy at the Kynet/Quinn ownership boundary.
//!
...
#[derive(Clone)]
struct StationConnectRateController {
...
impl StationConnectRateController {
...
impl Controller for StationConnectRateController {
    fn on_ack(
        &mut self,
        now: Instant,
        sent: Instant,
...
/// Constructs one StationConnect rate controller per Quinn path.
pub struct StationConnectRateControllerFactory {
    policy: Arc<TransportRatePolicy>,
}

impl StationConnectRateControllerFactory {
    pub fn new(policy: Arc<TransportRatePolicy>) -> Arc<Self> {
        Arc::new(Self { policy })
    }
}

impl ControllerFactory for StationConnectRateControllerFactory {
    fn build(self: Arc<Self>, _now: Instant, current_mtu: u16) -> Box<dyn Controller> {
        Box::new(StationConnectRateController::new(
            self.policy.clone(),
            current_mtu,
        ))
    }
...
mod tests {
    ...
    fn repairable_random_loss_does_not_collapse_the_window() {
        ...
        let factory = StationConnectRateControllerFactory::new(policy);
        let now = Instant::now();
