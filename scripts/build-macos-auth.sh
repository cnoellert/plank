...
xcrun clang -mmacosx-version-min=27.0 -std=c11 -Wall -Wextra -Wpedantic -Werror \
    -I"$source_root/host/macos/auth" "$source_root/tests/auth/macos-account-policy.c" \
...
xcrun clang -mmacosx-version-min=27.0 -fobjc-arc -Wall -Wextra -Werror \
    ...
    -I"$source_root/host/macos/auth" \
    "$source_root/host/macos/auth/account-verifier.m" \
    "$source_root/host/macos/auth/account-channel.m" \
