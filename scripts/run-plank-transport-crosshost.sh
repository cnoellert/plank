...
probe_client=${PLANK_TRANSPORT_CLIENT:?Set PLANK_TRANSPORT_CLIENT for your test environment}
probe_client_ip=${PLANK_TRANSPORT_CLIENT_IP:?Set PLANK_TRANSPORT_CLIENT_IP for your test environment}
probe_server_ip=${PLANK_TRANSPORT_SERVER_IP:?Set PLANK_TRANSPORT_SERVER_IP for your test environment}
probe_port=${PLANK_TRANSPORT_PORT:-47489}
probe_duration=${PLANK_TRANSPORT_DURATION:-10}
probe_bitrate=${PLANK_TRANSPORT_BITRATE_BPS:-150000000}
probe_loss=${PLANK_TRANSPORT_LOSS_PERCENT:-0}
probe_loss_pattern=${PLANK_TRANSPORT_LOSS_PATTERN:-random}
probe_token=plank-transport-crosshost
probe_tmp=$(mktemp -d /tmp/plank-transport-crosshost.XXXXXX)
remote_binary=/tmp/plank-transport-probe-$$
server_pid=
nft_table_created=false

for numeric_value in "$probe_port" "$probe_duration" "$probe_bitrate" "$probe_loss"; do
  if [[ ! "$numeric_value" =~ ^[0-9]+$ ]]; then
    echo "plank_transport numeric setting is invalid: $numeric_value" >&2
    exit 2
  fi
done
if (( probe_port < 1 || probe_port > 65535 || probe_duration < 1 || probe_bitrate < 1 || probe_loss > 100 )); then
  echo 'plank_transport port, duration, bitrate, or loss percentage is out of range' >&2
  exit 2
fi
if [[ "$probe_loss_pattern" != random && "$probe_loss_pattern" != periodic-burst ]]; then
  echo 'plank_transport loss pattern must be random or periodic-burst' >&2
  exit 2
fi

cleanup() {
  if [[ "$nft_table_created" == true ]]; then
    sudo -n nft delete table inet plank_transport_test 2>/dev/null || true
  fi
...
if sudo -n nft list table inet plank_transport_test >/dev/null 2>&1; then
  echo 'refusing to reuse the existing inet plank_transport_test nft table' >&2
  exit 1
fi
...
if (( probe_loss > 0 )); then
  sudo -n nft add table inet plank_transport_test
  nft_table_created=true
  sudo -n nft 'add chain inet plank_transport_test output { type filter hook output priority filter; policy accept; }'
  if [[ "$probe_loss_pattern" == random ]]; then
    sudo -n nft add rule inet plank_transport_test output \
