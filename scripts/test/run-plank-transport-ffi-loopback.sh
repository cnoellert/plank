...
probe_source="$repo_root/probes/network/plank-transport/ffi-loopback.c"
probe_tmp=$(mktemp -d /tmp/plank-transport-ffi.XXXXXX)
...
if [[ ! "$probe_port" =~ ^[0-9]+$ ]] || (( probe_port < 1 || probe_port > 65535 )); then
  echo "invalid plank_transport loopback port: $probe_port" >&2
  exit 2
fi
...
cc -std=c11 -Wall -Wextra -Wpedantic -Werror \
  -I"$crate_dir/include" \
  "$probe_source" \
  "$crate_dir/target/release/libplank_transport.a" \
  -ldl -lpthread -lm -lrt \
