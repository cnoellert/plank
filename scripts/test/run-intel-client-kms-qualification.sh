...
if [[ ${PLANK_ALLOW_DISPLAY_STOP:-no} != yes ]]; then
  echo "set PLANK_ALLOW_DISPLAY_STOP=yes to authorize the isolated KMS run" >&2
  exit 2
fi

repo_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
build_dir=${PLANK_CLIENT_BUILD_DIR:-"${repo_dir}/build/client-qualification"}
PLANK_BUILD_ONLY=1 "${repo_dir}/scripts/test/probe-intel-client-kms.sh" "$@"
