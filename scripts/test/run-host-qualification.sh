...
overall_result=0
{
  ...
  "${repo_dir}/scripts/test/probe-nvenc-hevc44410.sh" 2>&1 || overall_result=1
  echo '```'
  echo
  echo '## Wacom Host Inventory'
  echo
  echo '```text'
  "${repo_dir}/scripts/test/probe-wacom-host.sh" 2>&1 || overall_result=1
  echo '```'
  echo
  ...
  echo
  echo '```text'
  "${repo_dir}/scripts/test/probe-pam-policy.sh" 2>&1 || overall_result=1
  echo '```'
  echo
  echo '## Result'
  echo
