...
probe_source="$repo_root/probes/network/datasmash/native-ffi-loopback.c"
probe_tmp=$(mktemp -d /tmp/stationconnect-native-ffi.XXXXXX)
...
cc -std=c11 -Wall -Wextra -Wpedantic -Werror \
  -I"$crate_dir/include" \
  "$probe_source" \
  "$target_dir/release/libstationconnect_datasmash_transport.a" \
  -ldl -lpthread -lm -lrt \
