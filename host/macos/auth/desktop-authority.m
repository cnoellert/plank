...
static BOOL readDesktop(PLANKMacAccountIdentity *account, SecuritySessionId *sessionID) {
    memset(account, 0, sizeof(*account));
    if (!geteuid() || getuid() != geteuid()) return NO;
    ...
    if (!session || session[(__bridge NSString *)kCGSessionOnConsoleKey] != (__bridge id)kCFBooleanTrue ||
        session[(__bridge NSString *)kCGSessionLoginDoneKey] != (__bridge id)kCFBooleanTrue) return NO;
...
- (instancetype)init {
    ...
    @synchronized(self) {
        if (!_notified && readDesktop(&_initial.account, &_sessionID) &&
